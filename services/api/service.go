...
func (api *RelayAPI) handleRegisterValidator(w http.ResponseWriter, req *http.Request) {
	...
	_, err = jsonparser.ArrayEach(body, func(value []byte, dataType jsonparser.ValueType, offset int, _err error) {
		...
		metadata := registrationMetadata{
			...
			Timestamp:  uint64(signedValidatorRegistration.Message.Timestamp.UnixMilli()),
			UserAgent:  ua,
		}
...
func (api *RelayAPI) handleGetPayload(w http.ResponseWriter, req *http.Request) {
	...
	// Although getPayloadWithFallbacks should always return a payload or an err, this code is to critical to assume. We check it hasn't returned nil without err anyway.
	if getPayloadResp == nil {
	...
	msNeededForPublishing = uint64(firstPublishTime - timeBeforePublish) //nolint:gosec
